...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='C:\\Users\\Dell\\Desktop\\AC\\program\\AC-test_lastrun.py',
...
def run(expInfo, thisExp, win, inputs, globalClock=None, thisSession=None):
    ...
    # --- Initialize components for Routine "gender_selection" ---
    Gender = visual.Slider(win=win, name='Gender',
        startValue=None, size=(0.6, 0.1), pos=(0, 0.1), units=win.units,
        labels=("female", "male", "diverse"),ticks=None, granularity=1,
    ...
    # --- Initialize components for Routine "Age_selection" ---
    ...
    Age = visual.TextBox2(
         ...
         color=[0.3255, 0.3255, 0.3255], colorSpace='rgb',
         opacity=None,
    ...
    # --- Prepare to start Routine "gender_selection" ---
    continueRoutine = True
    # update component parameters for each repeat
    thisExp.addData('gender_selection.started', globalClock.getTime())
    Gender.reset()
    key_resp_2.keys = []
    key_resp_2.rt = []
    _key_resp_2_allKeys = []
    # keep track of which components have finished
    gender_selectionComponents = [Gender, text_2, key_resp_2, text_3]
    for thisComponent in gender_selectionComponents:
        thisComponent.tStart = None
        thisComponent.tStop = None
        thisComponent.tStartRefresh = None
        thisComponent.tStopRefresh = None
    ...
    # --- Ending Routine "gender_selection" ---
    for thisComponent in gender_selectionComponents:
        if hasattr(thisComponent, "setAutoDraw"):
            thisComponent.setAutoDraw(False)
    thisExp.addData('gender_selection.stopped', globalClock.getTime())
    thisExp.addData('Gender.response', Gender.getRating())
    # check responses
    ...
    thisExp.nextEntry()
    # the Routine "gender_selection" was not non-slip safe, so reset the non-slip timer
    routineTimer.reset()
    
    # --- Prepare to start Routine "Age_selection" ---
    continueRoutine = True
    # update component parameters for each repeat
    thisExp.addData('Age_selection.started', globalClock.getTime())
    key_resp_3.keys = []
    key_resp_3.rt = []
    _key_resp_3_allKeys = []
    Age.reset()
    # keep track of which components have finished
    Age_selectionComponents = [text_4, key_resp_3, text_5, Age]
    for thisComponent in Age_selectionComponents:
        thisComponent.tStart = None
        thisComponent.tStop = None
        thisComponent.tStartRefresh = None
        thisComponent.tStopRefresh = None
    ...
    while continueRoutine:
        ...
        if key_resp_3.status == STARTED and not waitOnFlip:
            theseKeys = key_resp_3.getKeys(keyList=['space'], ignoreKeys=["escape"], waitRelease=True)
    ...
    # --- Ending Routine "Age_selection" ---
    for thisComponent in Age_selectionComponents:
        if hasattr(thisComponent, "setAutoDraw"):
            thisComponent.setAutoDraw(False)
    thisExp.addData('Age_selection.stopped', globalClock.getTime())
    # check responses
    ...
    # the Routine "Age_selection" was not non-slip safe, so reset the non-slip timer
    routineTimer.reset()
    
    # set up handler to look after randomisation of conditions etc
    trials = data.TrialHandler(nReps=1, method='random', 
        extraInfo=expInfo, originPath=-1,
        trialList=data.importConditions('facesBlock.xlsx'),
    ...
    for thisTrial in trials:
        ...
        continueRoutine = True
        # update component parameters for each repeat
        thisExp.addData('variable_globalclock.started', globalClock.getTime())
        ...
        for thisComponent in variable_globalclockComponents:
            if hasattr(thisComponent, "setAutoDraw"):
                thisComponent.setAutoDraw(False)
        thisExp.addData('variable_globalclock.stopped', globalClock.getTime())
        ...
        while continueRoutine and routineTimer.getTime() < 10.1:
            ...
            if movie.status == NOT_STARTED and tThisFlip >= 0.1-frameTolerance:
                # keep track of start time/frame for later
            ...
            if movie.status == STARTED:
                ...
                if tThisFlipGlobal > movie.tStartRefresh + 10-frameTolerance:
        ...
        continueRoutine = True
        # update component parameters for each repeat
        thisExp.addData('variable.started', globalClock.getTime())
        ...
        for thisComponent in variableComponents:
            if hasattr(thisComponent, "setAutoDraw"):
                thisComponent.setAutoDraw(False)
        thisExp.addData('variable.stopped', globalClock.getTime())
        ...
        continueRoutine = True
        # update component parameters for each repeat
        thisExp.addData('Emotion_level.started', globalClock.getTime())
        slider_2.reset()
        text_6.setText(msg)
        key_resp_4.keys = []
        key_resp_4.rt = []
        _key_resp_4_allKeys = []
        # keep track of which components have finished
        Emotion_levelComponents = [slider_2, text_6, key_resp_4, text_7]
        ...
        for thisComponent in Emotion_levelComponents:
            if hasattr(thisComponent, "setAutoDraw"):
                thisComponent.setAutoDraw(False)
        thisExp.addData('Emotion_level.stopped', globalClock.getTime())
